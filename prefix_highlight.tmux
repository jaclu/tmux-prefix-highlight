...
main() {
    ...
    if [[ "$sync_supported" = "Yes" ]]; then
        local -r show_sync_mode=$(tmux_option "$show_sync_config" "off")
        local -r sync_attr=$(tmux_option "$sync_attr_config" "$default_sync_attr")
        local -r sync_prompt=$(tmux_option "$sync_prompt" "$default_sync_prompt")
